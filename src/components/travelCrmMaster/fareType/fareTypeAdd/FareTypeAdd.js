...
import { addafareType, getByfareType, updatefareType } from "../../../../api/login/Login";

const FareTypeAdd = () => {
    const breadCrumbsTitle = {
        id: "1",
        title_1: "Travel CRM Master",
        title_2: 'Travel Fare Type Add',
        path_2: ``
    };
    ...
    const submitData = async () => {
        ...
        if (!params?.id) {
            try {
                ...
                const res = await addafareType(clone)
                if (res?.error == false) {
                    toastSuccessMessage()
                    setTimeout(() => {
                        navigate(`/travel-fare-type`)
                    }, 2000)
                } else {
                    alert(res?.message)
                }
            ...
            try {
                const res = await updatefareType(params.id, clone)
                if (res?.error == false) {
                    toastSuccessMessage()
                    setTimeout(() => {
                        navigate(`/travel-fare-type`)
    ...
    useEffect(() => {
        const detbyIdData = async () => {
            try {
                const res = await getByfareType(params?.id)
                    ...
                    <div className="card-body p-0">
                        <div className="table-responsive active-projects style-1">
                            <div className="tbl-caption tbl-caption-2">
                                <h4 className="heading mb-0 p-2">{params?.id ? 'Update' : 'Add'} Travel Fare Type </h4>
                            </div>
                            <form className="tbl-captionn">
                                <div className="row">
                                    <div className="col-xl-4 mb-3">
                                        <label for="exampleFormControlInput1" className="form-label">Fare Type Name <span style={{ color: 'red' }}>*</span></label>
                                        <input
                                            type="text"
