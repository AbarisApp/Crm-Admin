...
import FareTypeFilter from "./fareTypeFilter/FareTypeFilter";
import { deletefareType, gitfareType } from "../../../api/login/Login";
...
const FareType = () => {
    ...
    const getTransitionReport = async (input) => {
        ...
        try {
            const res = await gitfareType(clone)
    ...
    const deleteData = async (id) => {
        try {
            const res = await deletefareType(id)
            // console.log(res);
            if (res?.error == false) {
                toastSuccessMessage()
    ...
    return (
        <>
            ...
            <FareTypeFilter filterInitial={filterInitial} handleChange={handleChange} getTransitionReport={getTransitionReport} />
            <div>
                            ...
                            <div className="card-body p-0">
                                <div className="table-responsive active-projects style-1">
                                    <div className="tbl-caption">
                                        <h4 className="heading mb-0 p-2">Fare Type Master</h4>
                                        <Link to='/travel-fare-type-add' className="btn btn-primary">Add Fare Type Master</Link>
                                    </div>
                                        ...
                                        <table id="banner-tblwrapper" className="table dataTable no-footer" role="grid" aria-describedby="banner-tblwrapper_info">
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <th style={{ width: '150px' }}>Fare Type Name</th>
                                                    ...
                                                    return <tr role="row" key={item?._id}>
                                                        ...
                                                        <td>
                                                            <div className="d-flex">
                                                                <Link to={`/travel-fare-type-update/${item?._id}`} className="btn btn-primary shadow btn-xs sharp me-1">
