...
function CreateConvertForm
    ...
    const submitForm = async (values) => {
        try {
            if (!params?.id) {
                try {
                    const res = await postCallConvertStatus(values);
                ...
                try {
                    const res = await updateCallConvertStatusById(params.id, values);
...
export default CreateConvertForm
