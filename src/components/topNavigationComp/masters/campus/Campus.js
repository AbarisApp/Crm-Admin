...
import CampusFilter from "./campusFilter/CampusFilter";
import Breadcrumbs from "../../../../common/breadcrumb/Breadcrumbs";
import { deletetCampus, getatrcrm, getcampus } from "../../../../api/login/Login";

const Campus = () => {
    const breadCrumbsTitle = {
        id: "1",
        title_1: "College",
        title_2: 'Campus Master',
        path_2: ``
    };
    ...
    const getTransitionReport = async (input) => {
        ...
        try {
            const res = await getcampus(clone)
            if (res?.error == false) {
    ...
    const deleteData = async (id) => {
        try {
            const res = await deletetCampus(id)
            // console.log(res);
            if (res?.error == false) {
                toastSuccessMessage()
    ...
    return (
        <>
            ...
            <CampusFilter filterInitial={filterInitial} handleChange={handleChange} getTransitionReport={getTransitionReport} />
            <div>
                            ...
                            <div className="card-body p-0">
                                <div className="table-responsive active-projects style-1">
                                    <div className="tbl-caption">
                                        <h4 className="heading mb-0 p-2">Campus Master
                                        </h4>
                                        <Link to='/campus-add' className="btn btn-primary">Add Campus Master</Link>
                                    </div>
                                                    ...
                                                    return <tr role="row" key={item?._id}>
                                                        ...
                                                        <td>
                                                            <div className="d-flex">
                                                                <Link to={`/campus-update/${item?._id}`} className="btn btn-primary shadow btn-xs sharp me-1">
